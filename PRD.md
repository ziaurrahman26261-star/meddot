...
# **0. What Changed in v2 — What / How / Why**

> **Nothing about the product itself changed.** Every portal, feature, and business rule in §4–§5 is exactly as v1 intended (sessions, classes, students, teachers, fees, attendance, marks, diary, timetable, promotion workflow, fee-defaulter blocking, one-login-per-child). What changed is the *mechanism* underneath: identity, data model, integrity guarantees, and cost plan. Each change below fixes a concrete defect or risk found in v1; full DDL and RLS policies are in `ARCHITECTURE.md`.

| # | Area | v1 (was) | v2 (how it works now) | Why it changed |
|---|---|---|---|---|
| 1 | **Authentication** | NextAuth.js v5 (Credentials Provider) signed its own session JWT | **Supabase Auth** (email + password). The `role@schoolSlug` login id is stored as the auth email; `role` and `school_id` live in the JWT's `app_metadata` claims; every request carries that JWT and RLS reads the claims directly | NextAuth's JWT is **not a Supabase JWT**, so RLS had no identity to enforce against — teams respond by disabling RLS, and one leaked key becomes a cross-school data leak. One identity source is simpler and safer |
| 2 | **Parent accounts** | `parent_password_hash` stored on `students` + separate `parent_users` table | One `users` row per person (admin/teacher/parent) with a single password; `parent_users(user_id, student_id)` links a parent login to a child | Two parallel password stores drift apart and double the attack surface. Unified identity also enables multi-child parent accounts later without migration |
| 3 | **Class membership** | `students.current_class_id` — the only record of a student's class | New `enrollments(student_id, session_id, class_id, status, joined_on, left_on)` table. Current class = the active enrollment; transfer/promotion create/update enrollment rows; DB enforces `UNIQUE (student_id, session_id)` | `current_class_id` destroys history (impossible to answer "which class in 2024-25?" after a promotion) and nothing prevents a student existing in two classes at once |
| 4 | **One batch voucher per class per month** | Checked in app code only | `class_id` added to `fee_vouchers`; **partial unique index** `(school_id, class_id, session_id, month_year) WHERE type='batch'` — the database refuses a second batch voucher | App-only rules fail on double-clicks, races, and future code paths. A DB constraint holds forever |
| 5 | **Fee balances & status** | `paid_amount` and `status` updated by app code on each payment | Payments insert into `fee_payments`; a **DB trigger recomputes `paid_amount`** in the same transaction; `status` (unpaid/partial/paid) is a **generated column** derived from amounts | App-managed balances can double-count or drift under concurrency; trigger + generated-column math is atomic and can never disagree with the payment rows |
| 6 | **Attendance** | Per-student saves, no uniqueness | One **batch UPSERT** per class per day + `UNIQUE (student_id, session_id, date)` | 1 statement instead of N (faster, atomic, idempotent) and duplicate marking is impossible |
| 7 | **Audit trail** | App-level logging in code | **DB triggers** write `audit_logs` on marks, fees, attendance, enrollments, and status changes; logins are audited too | App-level logging can be skipped by bugs or bypassed by direct DB access; triggers run inside the database and cannot be skipped |
| 8 | **Promotion / demotion / repeat / transfer** | Several app-level writes in sequence; fee check done *before* the writes | One **`SECURITY DEFINER` RPC** per operation: verifies caller role + school from JWT claims, checks fees **with row locks inside the transaction**, moves the enrollment, archives empty classes, writes the audit row — all or nothing | A partial failure mid-sequence could **lose a student between sessions**; and check-then-act fee validation was racy (a payment could land between check and move) |
| 9 | **Archived sessions** | UI hid the edit buttons | A **DB trigger rejects any write** referencing an archived session | UI-only rules don't survive bugs or API misuse; read-only archival is now guaranteed at the data layer |
| 10 | **Real-time** | Supabase Realtime + TanStack Query | TanStack Query **polling** (30–60 s intervals, refetch on window focus and after mutations); Vercel data cache (60 s TTL per school) for dashboards | Realtime is billed **per message** and adds connection complexity; a school app doesn't need sub-second push. Polling is free, testable, and with caching keeps egress inside free-tier limits |
| 11 | **Hosting & cost** | "Vercel" (implicitly the free Hobby plan) | Tiered plan: Vercel Pro + Supabase Free ≈ **$22/mo** → + Supabase Pro ≈ **$47/mo** → hybrid VPS ≈ **$8–10/mo**; upgrade triggers and guardrails defined | Vercel Hobby is **non-commercial by ToS** — a paid SaaS cannot legally run on it. Without a tier plan, egress/storage surprises produce bills; the ladder keeps cost ≤ $45/mo through ~100 schools |
| 12 | **Backups & DR** | None | Nightly `pg_dump` → **encrypted Backblaze B2** (30-day daily + 12-month monthly retention), restore runbook in repo, **quarterly restore drill**; Pro tier adds Supabase daily backups + PITR | Supabase **Free tier has no backups** — losing years of fee/attendance data is unrecoverable and fatal to school trust |
